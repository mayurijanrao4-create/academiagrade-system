...
export function downloadStudentScorecardPdf(
  ...
  doc.text('OFFICIAL ACADEMIC GRADE CARD & PERFORMANCE REPORT (SEM IV)', pageWidth / 2, 24, { align: 'center' });
  ...
  // Save PDF file
  const fileName = `GradeCard_${student.rollNumber.replace(/[^a-zA-Z0-9]/g, '_')}.pdf`;
  doc.save(fileName);
}
